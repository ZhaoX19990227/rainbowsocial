...
func (r *UserRepository) GetByID(id int64) (*model.User, error) {
	row := r.db.QueryRow(`
		SELECT id, account, email, nickname, avatar, photos, age, bio, tags, lat, lng, location_label, online_status, created_at, last_active_at
		FROM users
		WHERE id = ?
	`, id)
	return scanUser(row)
...
func (r *UserRepository) Create(email, account, nickname, passwordHash string) (*model.User, error) {
	...
	result, err := r.db.Exec(`
		INSERT INTO users (email, account, password_hash, nickname, avatar, photos, age, bio, tags, lat, lng, location_label, online_status, created_at, last_active_at)
		VALUES (?, ?, ?, ?, '', '[]', 18, '', '[]', 0, 0, '', 0, ?, ?)
...
func (r *UserRepository) UpdateProfile(user *model.User) (*model.User, error) {
	...
	_, err = r.db.Exec(`
		UPDATE users
		SET nickname = ?, avatar = ?, photos = ?, age = ?, bio = ?, tags = ?, lat = ?, lng = ?, location_label = ?, last_active_at = ?
		WHERE id = ?
	`, user.Nickname, user.Avatar, string(photosJSON), user.Age, user.Bio, string(tagsJSON), user.Lat, user.Lng, user.LocationLabel, time.Now().UTC(), user.ID)
	if err != nil {
		return nil, err
	}
